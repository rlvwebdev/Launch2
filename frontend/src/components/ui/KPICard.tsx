      ...
      <CardContent className="p-6">
        ...
        {target && typeof value === 'number' && (
          <div className="mb-4">
            ...
            </div>            <div className="w-full bg-[var(--color-neutral)]/20 rounded-full h-2">
              <div 
                className={cn(
                  "bg-[var(--color-accent)] h-2 rounded-full transition-all duration-300",
                  getProgressPercentage() > 90 ? "animate-pulse" : ""
                )}
        ...
        {/* Trend Mini Chart */}
        {variant === 'detailed' && trend && trend.length > 0 && (
          <div className="mt-4 pt-4 border-t border-[var(--color-neutral)]/20">
            <div className="flex items-end justify-between h-8 gap-1">
              {trend.map((point, index) => (
                <div
                  key={index}
                  className="bg-[var(--color-accent)]/20 rounded-sm min-w-[2px] flex-1"
                  style={{
                    height: `${(point.value / Math.max(...trend.map(p => p.value))) * 100}%`
                  }}
                />
              ))}
            </div>
            <p className="text-xs text-[var(--color-text-secondary)] mt-1 text-center">
              Trend over {trend.length} periods
            </p>
          </div>
        )}
      </CardContent>
